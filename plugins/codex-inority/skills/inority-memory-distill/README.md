...
`inority-memory-distill` 是 `codex-inority` 插件内的 backlog 蒸馏 skill。它面向 `.codex/memory/dairy/` 中已经落盘的历史 note，把单篇或最旧 note 处理成 USER、WORKSPACE、SOUL、对应领域 skill 或 credential 候选。
