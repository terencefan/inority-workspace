...
- the user explicitly invokes `inority-memory-maintenance` and clearly asks to process `.codex/memory/dairy/`
...
- `🔴` -> `.codex/memory/credential.md` or the matching `.codex/memory/credential.d/*.yaml` fragment

Use the same strict boundaries as `inority-memory-maintenance`: personal preference, team-wide preference, agent style, domain skill, or credential. Do not promote workflow and repair rules into SOUL, USER, or WORKSPACE.

## Resource Files

- `scripts/find-oldest-dairy.mjs`
- `references/distill-surface.md`
