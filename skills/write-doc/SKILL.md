...
5. 每次修改文档后都必须使用对应 mode 的 validator，不得只在最终定稿时运行。统一执行
   `scripts/docctl validate <path>`（`validate` 可省略），这里的 `scripts/docctl` 指
   `inority-workspace/skills/write-doc/scripts/docctl`。一次修改涉及多份文档时逐份校验；
   任一文档未通过时不得宣称完成。对应规则与错误码统一来自当前 mode 目录下的
   `validator/` 资产。若 validator 当前无法运行，再退回到模板/模式规则 + 代码事实
   交叉校验，并在结果里明确说明“未运行 validator”及原因。Markdown 内嵌 DOT 的专项
   校验命令属于 `$draw-dot` 的 `dotctl validate-markdown <path>`。
