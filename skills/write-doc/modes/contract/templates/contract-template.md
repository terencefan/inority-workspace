...
对 event / payload contract，默认优先用 `jsonc` code block 直接表达 authority JSON shape；表格只做索引或补充说明。

#### `<RunCreated>`

```jsonc
{
  "run_id": "run_123", // authority run id；由 scheduler / backend 生成
  "event_type": "run.created", // canonical event discriminator
  "payload": {
    "dataset_path": "s3://bucket/prefix", // canonical 字段；consumer 应优先读取它
    "datasetPath": "s3://bucket/prefix" // 历史兼容别名；仅兼容期保留
  },
  "created_at": "2026-06-22T10:00:00Z" // ISO-8601 时间戳
}
```
...
| `event_type` | `string` | `yes` | `<producer>` | `<event discriminator>` |
