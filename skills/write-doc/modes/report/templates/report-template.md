<!--
统一报告模板。Benchmark 是主模板，适用于性能实验。
非压测 report 使用文件末尾的补充模板。创建文档时只保留对应模式的模板正文。
-->
...
Callout 必须明确写出一种结论分类：`显著正向`、`正向但不显著`、`中性`、`负向但不显著`、`显著负向` 或 `不相关（排除）`。已完成实验必须在分类后写相对控制基线的带符号百分比，例如 `显著正向（+12.3%）`、`中性（+0.2%）` 或 `显著负向（-8.1%）`。建议映射：显著正向用 `[!TIP]`，方向性或中性用 `[!NOTE]`，负向用 `[!WARNING]` / `[!CAUTION]`，不相关或无效实验用 `[!IMPORTANT]`。
...
<!-- 非压测 report 补充模板从这里开始。使用时删除上方 Benchmark 模板。 -->
