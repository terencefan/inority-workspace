...
- 产品 spec / 技术 spec 默认二级标题顺序固定为：
  `背景与现状 / 目标与非目标 / 风险与红线 / 边界与契约 / 架构总览 / 架构分层 / 模块划分 / 方案对比 / 验收标准 / 访谈记录 / 参考资料`
- `LLM 节点 spec` 默认二级标题顺序固定为：
  `背景与现状 / 目标与非目标 / 风险与红线 / Prompt 设计 / Context 设计 / 边界与契约 / 验收标准 / 访谈记录 / 参考资料`
- `spec 总纲` 默认二级标题顺序固定为：
...
- 默认只写目标状态 authority contract；只有用户明确要求时，才补现状差异或兼容期约束。
- `架构总览 / 架构分层 / 模块划分 / 方案对比` 不默认强加给 `LLM 节点 spec`；只有节点确实存在非写不可的拓扑或路线比较时才增加。
- `边界与契约` 下建议按 `输出契约 -> 字段映射 -> 本地规则边界 -> 失败契约` 组织。

### 架构总览 / 架构分层 / 模块划分

- `架构总览` 先给整体形状，再按自然子系统拆解；图是主叙事，文字是补充说明。
- `架构分层` 表达南北向结构：请求路径、运行位置、职责层级、网络链路、存储路径或多阶段流程。
- `模块划分` 表达东西向结构：业务模块、平台模块、共享服务、领域、命名空间、ownership 或责任平面。
- `模块划分` 下每个三级标题必须紧跟一个 GitHub note 风格 block：`> [!NOTE]` + 一句简洁说明，用来概括该模块的核心职责、边界或 ownership；不要让模块小节直接进入正文。
