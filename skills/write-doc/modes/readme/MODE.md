...
- 当项目 README 需要固定说明 S3、对象存储、数据湖、资源根目录或交付根目录时，可在
  `代码结构` 与 `部署拓扑` 之间增加可选二级标题 `存储布局`。该章节用于表达跨模块
  存储域和根前缀，不应混入运行命令或部署过程。
- `代码结构` 优先使用 `路径 | 说明` 表格。
...
- `模块简介` 必须包含一张小型模块脑图，使用 fenced `dot` / `graphviz`、`rankdir=TB`，并至少存在一条 root -> category -> leaf 路径，也就是根节点之下至少两级、整体至少 3 层。脑图表达目录内部组成与责任分组，不复制调用时序或完整架构图。
- Module README 脑图必须按 `$draw-dot` 的 cross-theme 规则显式设置节点填充、边框和文字颜色，并通过 Graphviz 实际渲染。
- 面向浏览器预览的中文脑图必须保留方块字形比例，并给节点设置安全最小宽度；不得依赖 `spacingAndGlyphs` 压缩汉字来适配节点。
- `相关文件` 优先用表格表达。
