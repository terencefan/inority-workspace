...
- 单独 benchmark 一个组件时，必须同时隔离并 mock 它的上游与下游，不得把真实相邻组件的排队、算力或失败混入组件结论。上游并发 mock/loadgen 固定使用 Go 实现，并复现真实请求契约、数据分布、到达模型和并发/背压特征；下游 mock 必须运行在独立 Pod 和独立资源配额中，复现响应契约、payload 大小、延迟/抖动、吞吐上限和错误/背压语义，并通过调度隔离避免与被测组件争抢节点资源。实验前登记所有 mock 资源及清理 allowlist，实验后删除并确认 mock、客户端和临时服务归零。
...
- 进入 profiling、火焰图、热点、GIL、锁竞争、heap、系统调用或语言迁移判断时，按需完整加载 [`modules/profiling.md`](modules/profiling.md)；普通 benchmark 不加载该模块。
