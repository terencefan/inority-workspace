...
- 一级标题使用 `<对象> <实验主题> Benchmark`，不添加日期、实验 ID 或实验批次范围。日期由正文元数据和目录路径表达。
...
- `实验组` 与 `实验结果` 的实验 ID 必须一一对应且顺序一致。
- 每个实验结果只使用一个 `### <实验 ID> <名称>`。首个内容块必须是含结论分类和带符号百分比的 callout。
- 实验卡片使用加粗字段、列表和 Markdown 表格，不增加 H4、H5 或 H6。
- 未知值写为 `测量中` 或 `未取得（具体原因）`，不得填写推测值。
- `参考资料` 位于最后。公式遵循 `../../references/formulas.md`。
