...
4. 统一排版表达。
   ...
   - 正文使用加粗引导式小标题时，小标题必须独占一段并保留加粗，末尾不带句号、冒号等标点；对应正文必须另起下一段。禁止写成 `<p><b>工作背景与目标。</b> 正文</p>`，应写成 `<p><b>工作背景与目标</b></p><p>正文</p>`。
...
- 飞书文档中的普通 `<table>` 默认应拉齐页面宽度。发布或回读验证时，如果发现表格窄于页面主体宽度，应优先调整表格布局、列数或列宽，让表格宽度与 page width 对齐。
