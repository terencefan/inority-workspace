...
- 该项的 `#### 验收`
- `#### 验收` 的通过条件
- `## 最终验收` 中与该编号项相关的 checkbox（只读参考，不在本 lane 内勾选）
- 停止规则
...
- 这只是控制面收数
- 你要优先把当前验收是否仍可能通过、当前停在哪个检查、以及 blocker 讲清楚
- 不要借这个中断擅自切到别的 item
...
- 严格按写好的验收计划执行
- 每次检查前，都重新读一遍 `#### 验收` 的通过 / 失败条件；`## 最终验收` 的 checkbox 只作为最终汇总入口参考
- 把实际结果与计划中的通过条件逐项比对
- 如果当前 item 需要调用 `runctl` 写回 authority，可以先批量准备或审阅待执行命令，但真正执行时必须串行执行这次 `runctl`：不要后台跑，不要和其他 lane 的 `runctl` 并发，也不要在结果未确认前继续下一次 authority 写入
...
- 不要在本 lane 内勾 `## 最终验收` 的 checkbox；最终 checkbox 由主控在最终验收收口时统一更新
- 保留已经写在那里的执行证据
