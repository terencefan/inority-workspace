...
8. 独立最终验收通过后，使用该 recon 的终态证据更新 `## 参考资料` 关联的对应 spec；spec 对齐完成后才允许报告 Runbook 完成
9. 需要更新多个互不冲突的 spec 时，可以按 spec 并行启动多个 subagent；主 agent 必须回收全部结果，检查跨 spec 口径与链接一致性，并完成最终验收收口
...
1. 第一段只说明停止原因，包括失败项、预期、实际结果和停止边界。
2. 第二段只说明修订内容，包括已完成的现场清理、authority 修改，以及重新进入执行态前仍需完成的修订。

不要把停止原因与修订内容混在同一段，也不要只报告“已停止”而省略其中任一段。
