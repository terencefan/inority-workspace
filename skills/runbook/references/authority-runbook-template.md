# Authority Runbook Templates

模板已经按模式收口到通用 runbook 模板。

请选择对应模板：

- 通用模式： [runbook-template.md](./runbook-template.md)
  - 适用于 `coding` / `operation` / `migration`
- slides / PPT / H5 规划：请改用独立 skill `$inority-slides`

兼容说明：

- 这个文件保留为索引入口，避免旧链接直接失效
- 新增或修改模板时，不要再把模板正文写回这个文件
