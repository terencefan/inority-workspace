...
- `slides` -> `references/slides-runbook.md`
- 如果 authority 需要真正输出 `### 现状`、`### 目标` 或 `## 思维脑图` 的 Graphviz DOT 图，也可以按需加载 `$draw-dot`

加载规则：

- 默认只加载一个主类型子文档，不要无差别把三个子文档都读进来。
- `slides` 不是普通文档写作的兜底模式；只有当用户明确要求“用 runbook 写 PPT / slide / 演示稿”时才触发 `slides`
...
- 再判断它属于哪一类主 runbook：
  ...
  - `slides`：仅当用户明确要求“用 runbook 写 PPT / slide / 演示稿”时使用；主问题是受众、叙事、素材、图表、页面顺序或演示稿产出如何把现状推进到可交付 slides
...
- 首个执行项必须随主类型变化：
  ...
  - `migration`：第一步是 `冻结现场`
  - `slides`：第一步是 `冻结素材`
  - `slides`：从第 2 步开始，每个编号项默认只对应一页 slide；不要把两页或跨页动作塞进同一个编号项
...
- 如果输入或当前执行上下文指向一份已经存在的 authority runbook，先检查它是否位于当前本地日期目录下，并且 title / 文件名是否只反映当前 authority 目标：
  ...
  - 标题下的模式 Note 必须存在，且内容必须与当前判定的 `coding` / `operation` / `migration` / `slides` 主类型一致；如果不一致，先同步修正
...
- [references/slides-runbook.md](./references/slides-runbook.md)
...
- `## 执行计划`
  - 每个编号项都包含 `#### 执行` 与 `#### 验收`
  - 首个编号项必须和当前模式一致：`coding` 先 `保证工作区干净`，`operation` / `migration` 先 `冻结现场`，`slides` 先 `冻结素材`
  - `slides` 模式下，每个编号项默认只产出或修改一页；如果需要改第 `N` 页，就单独写成一个第 `N` 页的编号项
...
- `## 执行计划`
  ...
  - `slides` 模式的第一个执行项先冻结素材、受众和交付边界
  - `slides` 模式从第 2 项开始默认一项一页；页标题、页目标和页内素材都在该编号项内闭合
  - 每个编号项都有明确的执行和验收内容
