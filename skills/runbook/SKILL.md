...
- `runctl` 视为 authority runbook 单文件写入口；同一份 `<runbook.md>` 上的多个 `runctl` 调用一律串行执行。
...
```bash
python3 scripts/runctl add-qa <runbook.md> --question "<Q>" --answer "<A>" --time "<访谈时间>" --impact "<影响面>"
```
