...
- 只要后续执行需要引用某个配置、代码 patch、manifest、Secret 模板、校验脚本或回滚脚本，这些资产就必须在规划态先准备完毕，并在 authority 中以文件路径或稳定资源名被引用；不要把“执行时再现场写出来”留给 run 阶段。
- 如果某个编号项当前还依赖未落盘的实现资产，该编号项就还没达到可执行标准；正确动作是继续停在规划态补齐资产，而不是提前切执行态。
