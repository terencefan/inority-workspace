...
- 在指定块内严格自上而下执行
- 默认把自己视为 rollout lane，而不是实现 lane：只 apply / restart / smoke / collect evidence 已在规划态准备好的资产，不在执行过程中现场补写配置、代码、模板或脚本。
...
如果执行中发现 authority 引用的 rollout 资产并不存在、内容明显不完整、或仍需要现场补写实现细节：

- 立即停止
- 不要在 execution lane 内自己把资产补出来
- 回报缺的是哪一类资产、authority 当前引用了什么、为什么不足以继续 rollout
- 明确要求主 rollout 回到规划态补齐资产后再重进执行态
