...
- 当需求涉及 `bitable` / `多维表格` / Base 数据内容、或需要通过 MCP 访问飞书资源时，默认依赖已安装的 `lark-mcp` skill（当前安装位置：`~/.codex/skills/lark-mcp`）。
...
- 如果工作区根目录 `~/workspace/docs/feishu/` 下还没有该文档的本地镜像，先从飞书拉取一份镜像，再开始后续编辑。
...
2. 锁定飞书目标和本地源稿。
   ...
   - 在工作区内为该文档保留稳定本地镜像路径，默认放在工作区根目录 `~/workspace/docs/feishu/`。
...
- authority XML 发布前，必须运行 `python3 skills/write-feishu-doc/scripts/validate_headings_callout.py <authority.xml>`。
...
- 对适合 Graphviz 表达的关系型图表，默认在工作区根目录 `~/workspace/docs/feishu/` 保留 `.dot` 源稿和对应渲染后的 `.svg` 发布稿。
- 如果文字已经足够清楚，不要为了“看起来完整”强行加图。

## 飞书发布纪律

- 优先使用本地 `lark-cli`。
- 把工作区根目录 `~/workspace/docs/feishu/` 下的本地镜像视为 authority，飞书成品是发布结果，不是唯一真相源。
...
- 发布前必须运行 `python3 skills/write-feishu-doc/scripts/validate_headings_callout.py <authority.xml>`；如果校验失败，先补齐缺失的 callout，再执行 Feishu 发布。
