...
- 每当需要向 Terence 提问、请求确认、请求授权、让 Terence 选择路径、收敛范围、确认风险、确认验收标准、确认是否继续、确认是否允许敏感操作，或回复中将出现任何问号/“是否/要不要/能否/可不可以/确认一下/你选/请选择/需要你确认”这类提问语气时，都必须先加载并使用 `inority-question` skill；加载动作必须发生在提出问题之前。
- 如果已经意识到需要提问，但当前回合还没有读取 `inority-question` 的 `SKILL.md`，必须先读取 `/home/fantengyuan/workspace/inority-workspace/skills/inority-question/SKILL.md`，再组织问题；不要凭记忆直接提问。
- 若多个 skill 同时适用，且最终需要向 Terence 提问或确认，`inority-question` 必须作为最后的提问表层 skill 一并加载，用它来格式化最终问题。
...
- 写飞书文档里的对比表格时，默认横向表头/各列是待对比对象（A/B/C...），纵向第一列是对比内容或维度（X/Y/Z...）。
