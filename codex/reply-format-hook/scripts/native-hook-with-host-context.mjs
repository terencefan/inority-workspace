...
const VSCODE_TEMPLATE = join(PACKAGE_ROOT, "references", "reply-format-vscode.md");
...
function resolveTemplatePath(hostInterface) {
  if (hostInterface === "vscode") {
    return VSCODE_TEMPLATE;
  }
...
function buildHostContextLine(hostInterface, templatePath) {
  if (hostInterface === "vscode") {
    return `Detected host interface: vscode. Use the editor-hosted reply template at ${templatePath}.`;
  }
...
function buildSessionStartContext(hostInterface, payload) {
  ...
  const sections = [
    ...
    "- Begin with Goal, Ambiguity, and Risk.",
    "- Ambiguity and Risk must use percentages.",
    "- Explain the highest current risk inline in the Risk description.",
    hostInterface === "vscode"
      ? "- In this session, render them with the VS Code Markdown table format."
...
function buildPromptSubmitContext(hostInterface, payload) {
  ...
  const userRulesPath = resolveUserRulesPath(payload);
  if (userRulesPath) {
    return `${buildHostContextLine(hostInterface, templatePath)} Reply-format rules remain governed by ${userRulesPath}.`;
  }
  return `${buildHostContextLine(hostInterface, templatePath)} Reply-format rules remain governed by the selected template for this session.`;
}
...
if (!nativeHookPath) {
  console.error("reply-format-hook: failed to resolve OMX native hook path");
  process.exit(1);
}
